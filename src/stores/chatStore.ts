...
import { streamMessage } from '../services/streamApi';
import { ChatListItem, Message } from '../types/chat';
import { handleLimitError } from '../utils/parseApiError';
...
export const useChatStore = create<ChatState>((set, get) => ({
    ...
    regenerateMessage: async (chatId) => {
        ...
        await chatService.regenerateMessageStream(chatId, {
                ...
                set((state) => ({
                    ...
                    messages: state.messages.map((m) => {
                        if (m.localId === aiLocalId) {
                            const safeId = (meta.message_id && meta.message_id !== '0')
                                ? meta.message_id
                                : m.id;
            ...
            onError: (err) => {
                ...
                if (handleLimitError(code) || handleLimitError(errorMessage)) {
                    ...
                    set((state) => ({
                        ...
                        messages: state.messages.map((m) => {
                            ...
                            if (m.localId === userLocalId) {
                                const safeUserId = (meta.user_message_id && meta.user_message_id !== '0')
    ...
    uploadFile: async (chatId, file) => {
        ...
        try {
            ...
            console.error(e);
            const errorMessage = e?.message || 'Erro ao enviar arquivo.';
            set((state) => ({
                error: errorMessage,
                isStreaming: false,
