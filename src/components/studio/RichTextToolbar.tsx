...
export const RichTextToolbar: React.FC<Props> = ({ onStylePress }) => {
  ...
  return (
    <View className="bg-space-light/95 border-t border-white/10 pb-6 pt-2">
        ...
        <View className="flex-row justify-around items-center px-4 h-12">
            <Pressable onPress={() => onStylePress('bold')} className="p-2 rounded-lg active:bg-white/10">
                <Bold color="#e2e8f0" size={24} />
            </Pressable>

            <Pressable onPress={() => onStylePress('italic')} className="p-2 rounded-lg active:bg-white/10">
                <Italic color="#e2e8f0" size={24} />
            </Pressable>

            <Pressable onPress={() => toggleMenu('color')} className="p-2 rounded-lg active:bg-white/10 relative">
                <Underline color="#e2e8f0" size={24} />
                <View className="h-1 w-full bg-cosmic-purple absolute bottom-1 rounded-full" />
            </Pressable>
